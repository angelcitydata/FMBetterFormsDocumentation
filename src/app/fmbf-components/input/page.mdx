...
- **`currency, number, percent`** → `inputmode="decimal"` (numeric keypad with decimal)
- **`integer, date, ssn`** → `inputmode="numeric"` (numeric keypad only)
- **`phone`** → `inputmode="tel"` (telephone keypad)
- **All others** → No inputmode (standard keyboard)
