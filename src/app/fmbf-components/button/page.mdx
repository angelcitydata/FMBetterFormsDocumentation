...
- **`variant`**: Visual style variant (default, primary, secondary, destructive, success, warning, info, outline, ghost, link)
- **`fullWidth`**: Whether button expands to full width
- **`width`**: Width behavior when fullWidth is false (fit or full)
- **`isDisabled`**: Whether button is disabled
- **`loadingKey`**: Model key to check for loading state
- **`namedAction`**: Action to trigger when clicked
