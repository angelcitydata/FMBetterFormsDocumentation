...
The complete component schema for the Accordion component. Copy and paste this code into the Component Schema section of your BetterForms app.
...
{
    "collapsible": true,
    "html": "<div class=\"w-full bg-card text-card-foreground border border-border rounded-lg divide-y divide-border overflow-hidden\">\n    <!-- Item -->\n    <div v-for=\"(item, idx) in schema.items\" :key=\"item.id\" class=\"group\">\n        <!-- Trigger -->\n        <button :id=\"(schema.idPrefix || 'acc') + '-' + item.id + '-trigger'\" :aria-controls=\"(schema.idPrefix || 'acc') + '-' + item.id + '-content'\" role=\"button\" :aria-expanded=\"!!(vueapp.$store.state.wndw.windows[schema.windowIndex].formSchema.model[schema.stateKey] || {})[item.id]\" :disabled=\"item.disabled\" @click=\"(function(){\n  var win = vueapp.$store.state.wndw.windows[schema.windowIndex];\n  var model = win.formSchema.model;\n  var k = schema.stateKey || 'accordionState';\n  var cur = (model[k] || {});\n  var isOpen = !!cur[item.id];\n  var t = schema.type || 'single';\n  var canCollapse = (schema.collapsible !== false);\n\n  var nextMap;\n\n  if (t === 'single') {\n    // Build the next state from scratch (atomic update)\n    if (canCollapse && isOpen) {\n      nextMap = {}; // collapse the only open item\n    } else {\n      nextMap = {};             // close others\n      nextMap[item.id] = true;  // open target\n    }\n  } else {\n    // multiple: toggle this one, keep others\n    nextMap = Object.assign({}, cur, { [item.id]: !isOpen });\n  }\n\n  // Single assignment avoids intermediate paints\n  model[k] = nextMap;\n})()\" class=\"w-full flex items-center justify-between gap-2 px-4 py-3 text-left hover:bg-accent hover:text-accent-foreground transition-colors disabled:opacity-50 disabled:pointer-events-none\">\n            <span class=\"text-base font-medium\">{{ item.title }}</span>\n            <i class=\"fa-regular fa-chevron-down transition-transform duration-200\" :class=\"((vueapp.$store.state.wndw.windows[schema.windowIndex].formSchema.model[schema.stateKey] || {})[item.id]) ? 'rotate-180' : 'rotate-0'\"></i>\n        </button>\n\n        <!-- Content -->\n        <div :id=\"(schema.idPrefix || 'acc') + '-' + item.id + '-content'\" role=\"region\" :aria-labelledby=\"(schema.idPrefix || 'acc') + '-' + item.id + '-trigger'\" class=\"overflow-hidden transition-all duration-300 will-change-[max-height,opacity]\" :class=\"((vueapp.$store.state.wndw.windows[schema.windowIndex].formSchema.model[schema.stateKey] || {})[item.id]) ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'\">\n            <div class=\"px-4 pb-4 text-base text-muted-foreground\">\n                {{ item.content }}\n            </div>\n        </div>\n    </div>\n</div>",
    "idPrefix": "acc",
    "items": [],
    "stateKey": "accordion",
    "type": "single",
...
<div class="w-full bg-card text-card-foreground border border-border rounded-lg divide-y divide-border overflow-hidden">
    <!-- Item -->
    <div v-for="(item, idx) in schema.items" :key="item.id" class="group">
        <!-- Trigger -->
        <button :id="(schema.idPrefix || 'acc') + '-' + item.id + '-trigger'" :aria-controls="(schema.idPrefix || 'acc') + '-' + item.id + '-content'" role="button" :aria-expanded="!!(vueapp.$store.state.wndw.windows[schema.windowIndex].formSchema.model[schema.stateKey] || {})[item.id]" :disabled="item.disabled" @click="(function(){
...
2. **Create New Component**: If the Accordion component is not already in your components list, click the **+New** button at the top left.
...
- **`name`**: Component name (must be "Accordion")
- **`type`**: Accordion mode - "single" (one panel open) or "multiple" (multiple panels open)
- **`collapsible`**: Whether panels can be closed in single mode (true/false)
- **`stateKey`**: Model path for storing accordion state
- **`idPrefix`**: Prefix for generating unique HTML element IDs
- **`windowIndex`**: Window index for multi-window applications (default: 0)
- **`items`**: Array of accordion items with `id`, `title`, `content`, and optional `disabled` properties
