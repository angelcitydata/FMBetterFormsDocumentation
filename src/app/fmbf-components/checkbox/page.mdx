...
- **`modelKey`**: Model key where checkbox state is stored
- **`label`**: Text label displayed next to the checkbox
- **`description`**: Optional descriptive text below the label
- **`disabled`**: Whether the checkbox is disabled
- **`indeterminate`**: Whether checkbox is in indeterminate state
- **`items`**: Array for future checkbox list functionality
- **`namedAction`**: Action to trigger when checkbox state changes
- **`actionOptions`**: Additional options for the named action
- **`windowIndex`**: Window index for multi-window applications (default: 0)
- **`id`**: Optional unique identifier for the checkbox element
- **`emptyText`**: Text displayed when no results found (for future use)
- **`placeholder`**: Placeholder text (for future use)
